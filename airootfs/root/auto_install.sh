...
pacstrap /mnt base linux linux-firmware grub sudo networkmanager nano vim
...
if [[ \"\$SUDOOPT\" == \"y\" ]]; then
  ...
  # Create sudoers drop-in file for wheel group
...
# echo "[+] Detecting internal disks..."
#
# # List disks bigger than 30GB (non-removable, non-usb)
# DISKS=($(lsblk -dno NAME,TYPE,SIZE | awk '
#   $2=="disk" {
#     gsub("G","",$3)
#     if ($3 >= 30) print $1
...
#   parted --script "$DISK" mkpart primary linux-swap 301MiB 4297MiB
#   parted --script "$DISK" mkpart primary ext4 4297MiB 100%
#   P1="${DISK}$(lsblk $DISK | grep part | awk NR==1'{print $NF}' | grep -Eo '[0-9]+$')"
#   PESP="${DISK}1"
#   PSWAP="${DISK}2"
#   PROOT="${DISK}3"
# else
